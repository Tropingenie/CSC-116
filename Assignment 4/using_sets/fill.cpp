/* fill.cpp
   ...
   Task: Using the provided class (in image.hpp), write a program that implements the "bucket fill" operation on
   a .png file. See 116f19_assignment4.pdf on Connex and block comments in code for more details.
...
void check_cardinals(Image& I, std::list<std::pair<unsigned, unsigned>>& Q, std::set<std::pair<unsigned, unsigned>>& S, const unsigned& x, const unsigned& y, const Pixel& original);
...
int main(int argc, const char** argv){
	...
	}catch(std::runtime_error& e){//bucket_fill() generates its own errors, though they are only for debugging
...
void bucket_fill (Image& I, const unsigned& start_x, const unsigned& start_y, const unsigned char & r, const unsigned char & g, const unsigned char & b){
	...
	*/
	...
	std::set<std::pair<unsigned, unsigned>> acknowledged;//Acknowledged is a set of all the points that have already been processed
	const Pixel original {I.at(start_x,start_y).r, I.at(start_x,start_y).g, I.at(start_x,start_y).b};//saves the original RGB values
	const Pixel desired {r, g, b};//saves the desired RGB values for comparison later
	...
	do{
		...
		check_cardinals(I, Q, acknowledged, x, y, original);//check the cardinal pixels
