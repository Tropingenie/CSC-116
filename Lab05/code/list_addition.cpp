...
std::list<int> add_lists(std::list<int> list1, std::list<int> list2) {
	...
	//Goal: Take two lists and add them -- ie, normal addition except each digit is an element
	auto iter1 = list1.rbegin();
	auto iter2 = list2.rbegin();
	std::list<int> sum;
	bool carry = 0;

	for (;iter1 != list1.rend(); iter1++, iter2++){
		int tmp = *iter1 + *iter2 + carry;
		carry = 0;
		if (tmp < 10) sum.push_front(tmp);
		else{
			tmp -= 10;
			carry = 1;
			sum.push_front(tmp);
