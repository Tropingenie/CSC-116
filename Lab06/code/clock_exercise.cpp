...
public:
   ...
   std::string str_h, str_m;
   int h, m;
   bool colon_passed {false}, fail{false};
   for(auto ch: time_string){
     if (ch == ':') colon_passed = true;
     else if (colon_passed) str_m += ch;
     else str_h += ch;
   }
   ...
   if (format24){
     ...
     if (is_pm() ) return( int_to_string(get_hour12(),2) + ":" + int_to_string(minute,2) + "pm" );
