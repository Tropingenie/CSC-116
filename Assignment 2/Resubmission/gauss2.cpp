...
//Following code is modified from ( https://www.sanfoundry.com/cpp-program-find-determinant-given-matrix/ )
double det(ull n, const Matrix & mat)//Checks the determinant of any nxn augmented matrix
{
    ull c, subi, i, j, subj;
	double d;
    Matrix submat; 
        ...
        {  
            ...
            {  
                ...
                {    
                    ...
                    submat.at(subi).at(subj) = mat.at(i).at(j);
                    subj++;
                }
                subi++;
            }
        d = d + (pow(-1 ,c) * mat.at(0).at(c) * det(n - 1 ,submat));
        }
    }
	//std::cout << d << std::endl;
    return d;
}
//Above code is modified from sanfoundry.com
...
int main(){
    ...
    try{
		...
		if (det(A.size(), A) < 0.0000001) throw exception ("Error: Matrix is ill-conditioned.");//Checks for singular matrices
