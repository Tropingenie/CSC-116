/* scrabble.hpp
   ...
   Definitions for the ScrabbleSolver class.

   You may modify this file, but ONLY in the places indicated. In particular, you must
   place the implementation of each function in the separate scrabble.cpp file.

   B. Bird - 08/03/2019

   Modified: Benjamin Lyne, V00914888 - 10/25/2019
*/
...
using UnorScoreMap = std::unordered_map< std::string, int >;
...
private:
    ...
    // Map the letters to their point values:
    static int letter_score(char ch){
        ...
        return letter_scores.at(ch);
    }

    UnorScoreMap dictionary_map {};//Since dictionary_map only has its elements added once, I will use an unordered map
};
